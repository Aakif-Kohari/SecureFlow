...
describe("redactValue", () => {
  ...
  it("caps a long array and says how much it dropped", () => {
    const serialized = JSON.stringify(
      redactMeta({ files: Array.from({ length: 200 }, (_, i) => `f${i}`) }),
    );
...
describe("resolveLogLevel", () => {
  it("honours an explicit LOG_LEVEL", () => {
    expect(
      resolveLogLevel({ NODE_ENV: "production", LOG_LEVEL: "warn" } as NodeJS.ProcessEnv),
    ).toBe("warn");
    expect(
      resolveLogLevel({ NODE_ENV: "production", LOG_LEVEL: " ERROR " } as NodeJS.ProcessEnv),
    ).toBe("error");
  });
  ...
  it("suppresses everything below error under test", () => {
    ...
    expect(resolveLogLevel({ NODE_ENV: "production", VITEST: "true" } as NodeJS.ProcessEnv)).toBe(
      "error",
    );
