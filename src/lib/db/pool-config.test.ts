...
describe("resolveConnectionString", () => {
  it("prefers the pooler URL", () => {
    expect(resolveConnectionString(env({ DATABASE_URL: DIRECT, DATABASE_POOL_URL: POOLED }))).toBe(
      POOLED,
    );
  });
  ...
  it("skips a blank pooler URL rather than losing the direct one", () => {
    expect(resolveConnectionString(env({ DATABASE_POOL_URL: "", DATABASE_URL: DIRECT }))).toBe(
      DIRECT,
    );
  });
...
describe("resolveMockDb", () => {
  ...
  it("allows it in development", () => {
    const decision = resolveMockDb(env({ NEXT_PUBLIC_MOCK_DB: "true", NODE_ENV: "development" }));
  ...
  it("allows it under test", () => {
    expect(resolveMockDb(env({ NEXT_PUBLIC_MOCK_DB: "true", NODE_ENV: "test" })).allowed).toBe(
      true,
    );
  });

  it("refuses it in a production runtime", () => {
    const decision = resolveMockDb(env({ NEXT_PUBLIC_MOCK_DB: "true", NODE_ENV: "production" }));
  ...
  it("refuses on a deployed server, where neither CI nor NEXT_PHASE is set", () => {
    ...
    expect(
      resolveMockDb(env({ NEXT_PUBLIC_MOCK_DB: "true", NODE_ENV: "production", PORT: "3000" }))
        .allowed,
    ).toBe(false);
  });
});
