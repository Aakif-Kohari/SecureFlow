...
describe("admitWebhook", () => {
  ...
  it("rejects a bad signature with 401", () => {
    expect(admitWebhook({ ...base, signatureHeader: `sha256=${"0".repeat(64)}` })).toMatchObject({
      ok: false,
      status: 401,
    });
  });
  ...
  it("rejects malformed JSON with 400 rather than 500", () => {
    const body = "{not json";
    expect(admitWebhook({ ...base, payloadText: body, signatureHeader: sign(body) })).toMatchObject(
      { ok: false, status: 400 },
    );
  });
  ...
  it("rejects a body whose Content-Length lied about being small", () => {
    expect(admitWebhook({ ...base, contentLength: "1", maxBytes: 4 })).toMatchObject({
      ok: false,
      status: 413,
    });
  });
