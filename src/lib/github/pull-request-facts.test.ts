...
describe("resolveAuthorAvatarUrl", () => {
  it("accepts an https avatar", () => {
    expect(resolveAuthorAvatarUrl(payload())).toBe("https://avatars.githubusercontent.com/u/1?v=4");
  });
...
describe("pullRequestUpdateData", () => {
  ...
  it("omits only the avatar when the login is present but the avatar is not", () => {
    const data = pullRequestUpdateData(buildPullRequestFacts(payload({ user: { login: "rio" } })));
