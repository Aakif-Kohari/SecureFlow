...
describe("the pool has an error listener", () => {
  ...
  it("swallows the event and logs it rather than rethrowing", async () => {
    ...
    expect(logged.some((entry) => entry.level === "error" && /idle/i.test(entry.message))).toBe(
      true,
    );
  });
...
describe("pool sizing", () => {
  ...
  it("warns about a value it had to change", async () => {
    ...
    expect(
      logged.some((entry) => entry.level === "warn" && /DB_POOL_MAX/.test(entry.message)),
    ).toBe(true);
  });
});
...
describe("NEXT_PUBLIC_MOCK_DB", () => {
  it("serves the mock client in development, loudly", async () => {
    ...
    expect(logged.some((entry) => entry.level === "warn" && /fabricated/.test(entry.message))).toBe(
      true,
    );
  });
...
describe("getPgPoolConfig", () => {
  it("accepts an override connection string", async () => {
    ...
    expect(mod.getPgPoolConfig("postgresql://other/db").connectionString).toBe(
      "postgresql://other/db",
    );
  });
