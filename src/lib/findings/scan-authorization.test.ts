...
describe("scanRequestSchema", () => {
  ...
  it("requires a non-empty repositoryId", () => {
    ...
    expect(scanRequestSchema.safeParse({ ...validBody, repositoryId: undefined }).success).toBe(
      false,
    );
  });
...
describe("scanJobVisibility", () => {
  ...
  it("hides a job with no repository, which has no owner to compare against", () => {
    expect(scanJobVisibility({ repositoryId: null, repository: null }, "user-1")).toBe("not-found");
  ...
  it("never answers visible for an empty user id", () => {
    expect(scanJobVisibility({ repositoryId: "repo-1", repository: { userId: "" } }, "")).toBe(
      "visible",
    );
    expect(
