...
const DEFAULT_PAYLOAD_SIGNATURES: PayloadSignature[] = [
  {
    ...
    pattern:
      /(?:eval|Function)\s*\(\s*(?:String\.fromCharCode|atob|decodeURIComponent|unescape|btoa)/i,
    severity: "CRITICAL",
    category: "RCE",
    version: "1.0.0",
    description:
      "Detects dynamic code execution wrapped in decoding primitives (eval/Function + charCode/atob).",
  },
  {
    id: "SIG-ZDAY-002",
    name: "Prototype Pollution Injection Payload",
    pattern:
      /__proto__\s*\[\s*['"]?[\w-]+['"]?\s*\]\s*=\s*|Object\.prototype\s*\[\s*['"]?[\w-]+['"]?\s*\]/i,
    severity: "HIGH",
...
export class DynamicFingerprintEngine {
  ...
  public analyzePayload(
    ...
    const matchedIds = matchedSignatures
      .map((s) => s.id)
      .sort()
      .join(",");
