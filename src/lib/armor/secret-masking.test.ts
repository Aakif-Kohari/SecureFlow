...
describe("provider-prefixed keys", () => {
  ...
  it("redacts a Slack token and an incoming webhook URL", () => {
    ...
    redacted(
      `https://hooks.${"sla" + "ck"}.com/services/T00000000/B00000000/XXXXXXXXXXXXXXXXXXXXXXXX`,
    );
  });
...
describe("authorization headers and connection strings", () => {
  ...
  it("redacts database passwords in URI connection strings while preserving database scheme", () => {
    const postgresUri =
      'DATABASE_URL = "postgres://app_user:superSecretPass123@db.production.internal:5432/app_db"';
  ...
  it("redacts Azure Storage Account connection strings", () => {
    const azureConn =
      "DefaultEndpointsProtocol=https;AccountName=prodstorage;AccountKey=abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrst==;EndpointSuffix=core.windows.net";
...
describe("false positive prevention (#669)", () => {
  ...
  it("does not redact CSS tokens, Tailwind classes, or color hex codes", () => {
    untouched(
      'const buttonClass = "bg-blue-500 hover:bg-blue-600 text-white rounded-lg shadow-md";',
    );
