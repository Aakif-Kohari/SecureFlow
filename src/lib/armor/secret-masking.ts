 ...
 */
const COLOR_OR_CSS_TOKEN_REGEX =
  /^(?:#[0-9a-f]{3,8}|rgba?\([^)]+\)|hsla?\([^)]+\)|(?:bg|text|border|p|m|gap|flex|grid|col|row|rounded|shadow|w|h)-[a-z0-9/_-]+)$/i;
...
const RULES: readonly MaskRule[] = [
  ...
  },
  {
    ...
    pattern:
      /(https?:\/\/)(?:[a-zA-Z0-9-]+\.)*(?:internal|local|corp|private|lan|cluster\.local)(?::\d+)?(\/[^\s"']*)?/gi,
    replace: (_m, scheme, _domain, path) =>
      `${scheme}[REDACTED_BY_THE_PROFESSOR:INTERNAL_URL]${path || ""}`,
  },
  {
    id: "url-userinfo",
    pattern: /(:\/\/[^@\s/]*:)([^@\s/]+)(@)/g,
  ...
  { id: "huggingface-token", pattern: /hf_[a-zA-Z0-9]{34,}/g },
  {
    ...
    pattern:
      /(?:Bot\s+|discord\s*[:=]\s*['"]?)[MNO][a-zA-Z0-9_-]{23,25}\.[a-zA-Z0-9_-]{6}\.[a-zA-Z0-9_-]{27,38}/g,
  },
  {
    id: "datadog-api-key",
  ...
  // ── Generic assignments ──────────────────────────────────────────────────
  {
    ...
    replace: (match, prefix: string, quote: string, value: string) =>
      NON_SECRET_VALUE.test(value.trim())
        ? match
 ...
 */
export function maskIngressFileContent(
  fileContent: string,
  options?: {
  ...
  if (opts.enableTargetedContextIsolation) {
    ...
    const targetedLines = lines.filter(
      (line) =>
        line.startsWith("+") ||
        line.startsWith("-") ||
        line.startsWith("@@") ||
        /^\s*(?:export|function|const|let|var|class|interface|type|public|private|async)\b/.test(
          line,
        ),
    );
