...
describe("getClientIp", () => {
  ...
  it("uses x-real-ip when the trusted proxy set no forwarded chain", () => {
    expect(
      getClientIp(h({ "x-real-ip": "5.6.7.8" }), { trustedHopCount: 1, trustedProxies: [] }),
    ).toBe("5.6.7.8");
  });
  ...
  it("trims whitespace from x-real-ip", () => {
    expect(
      getClientIp(h({ "x-real-ip": "  3.3.3.3  " }), { trustedHopCount: 1, trustedProxies: [] }),
    ).toBe("3.3.3.3");
  });
});
...
describe("checkRateLimit & withRateLimit — Redis fallback strategies", () => {
  ...
  it("fails open (returns true) when fallbackStrategy is fail-open and the check throws", async () => {
    const spy = vi
      .spyOn(redisModule, "checkRateLimitDetailed")
