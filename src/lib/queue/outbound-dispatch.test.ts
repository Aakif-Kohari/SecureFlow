...
describe("resolveDispatchConfig", () => {
  ...
  it("caps the timeout and ignores nonsense values", () => {
    expect(resolveDispatchConfig(env({ OUTBOUND_WEBHOOK_TIMEOUT_MS: "999999" })).timeoutMs).toBe(
      MAX_TIMEOUT_MS,
    );
    expect(resolveDispatchConfig(env({ OUTBOUND_WEBHOOK_TIMEOUT_MS: "soon" })).timeoutMs).toBe(
      DEFAULT_TIMEOUT_MS,
    );
    expect(resolveDispatchConfig(env({ OUTBOUND_WEBHOOK_TIMEOUT_MS: "-5" })).timeoutMs).toBe(
      DEFAULT_TIMEOUT_MS,
    );
  });
...
describe("assertDispatchableUrl", () => {
  ...
  it("rejects plaintext http by default and allows it when configured", async () => {
    await expect(
      assertDispatchableUrl("http://hooks.example.com/x", options),
    ...
    await expect(
      assertDispatchableUrl("http://hooks.example.com/x", permissive),
  ...
  it("skips every network check when private networks are explicitly allowed", async () => {
    ...
    await expect(
      assertDispatchableUrl("http://127.0.0.1:9002/hook", permissive),
    ).resolves.toBeInstanceOf(URL);
  });
});
...
describe("classifyResponseStatus", () => {
  ...
  it.each([301, 302, 307, 400, 401, 403, 404, 410, 422])("treats %i as permanent", (status) => {
...
describe("dispatchOutboundWebhook", () => {
  ...
  it("adds a timestamped signature when a secret is present", async () => {
    ...
    expect(
      verifySignatureHeader(headers[SIGNATURE_HEADER], "shhh", '{"a":1}', 300, 1_700_000_000),
    ).toBe(true);
  });
