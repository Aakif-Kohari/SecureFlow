...
import { getUserAuditLogFilters, getUserAuditLogs, getUserAuditLogsForExport } from "./audit";
...
describe("getUserAuditLogFilters (#659)", () => {
  ...
  it("does not serve one user the other user's filter list", async () => {
    mockGroupBy.mockResolvedValueOnce([{ action: "ALICE_ONLY" }]).mockResolvedValueOnce([]);
    await getUserAuditLogFilters();

    session = { user: { id: "user-2" } };
    mockGroupBy.mockResolvedValueOnce([{ action: "BOB_ONLY" }]).mockResolvedValueOnce([]);
...
describe("getUserAuditLogsForExport (#659)", () => {
  ...
  it("flags a truncated export instead of returning a bare array", async () => {
    ...
    mockFindMany.mockResolvedValue(Array.from({ length: MAX_EXPORT_ROWS }, (_, i) => logRow(i)));
