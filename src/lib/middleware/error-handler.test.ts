...
describe("scrubSensitiveData", () => {
  ...
  it("redacts connection strings", () => {
    const input =
      "Could not connect to postgresql://postgres:secretpassword@localhost:5432/secureflow";
    expect(scrubSensitiveData(input)).toBe("Could not connect to [REDACTED_CONNECTION_STRING]");
  });

  it("redacts windows path structure", () => {
    const input =
      "Error occurred in D:\\Struggle\\Open Source\\SecureFlow\\src\\app\\api\\route.ts";
...
describe("withErrorHandler middleware wrapper", () => {
  ...
  it("redacts database internal error details on 500 error", async () => {
    const handler = async () => {
      const err = new Error(
        "Prisma query failed: SELECT * FROM User WHERE codename = ...",
