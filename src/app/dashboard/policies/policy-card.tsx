...
export function PolicyCard({
  ...
  return (
    <Card
      className={`glass-card group relative overflow-hidden flex flex-col rounded-sm transition-all duration-300 ${optimisticActive ? "border-primary/40 shadow-[0_0_24px_hsl(var(--primary)/0.12)]" : "opacity-60 border-foreground/10"}`}
    >
      <div className="absolute inset-x-0 top-0 h-1 bg-primary/80" aria-hidden="true" />
      <div
        className="absolute inset-0 opacity-[0.06] [background-image:linear-gradient(hsl(var(--foreground)/0.45)_1px,transparent_1px),linear-gradient(90deg,hsl(var(--foreground)/0.45)_1px,transparent_1px)] [background-size:18px_18px] pointer-events-none"
        aria-hidden="true"
      />
      <div
        className="absolute -right-12 -top-12 h-28 w-28 rotate-45 border border-primary/20 bg-primary/5"
        aria-hidden="true"
      />
      ...
      <CardHeader className="relative z-[1] pt-7 pb-4">
        <div className="flex items-center justify-between gap-3 mb-5 pr-12 text-[9px] font-mono uppercase tracking-[0.24em] text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <FileKey2 className="w-3 h-3 text-primary" /> The Rules
          </span>
          <span className="flex items-center gap-1">
            <ScanLine className="w-3 h-3" /> Active file
          </span>
        </div>

        <div className="flex items-start gap-3 mb-3">
          <div
            className={`p-2.5 rounded-sm border ${optimisticActive ? "bg-primary/10 border-primary/30 text-primary" : "bg-muted border-border text-muted-foreground"}`}
          >
            <Lock className="w-4 h-4" aria-hidden="true" />
          </div>
          <Badge
            variant="outline"
            className={`mt-0.5 text-[10px] tracking-widest rounded-sm ${optimisticActive ? "border-primary/50 text-primary" : ""}`}
          >
            {action}
          </Badge>
        </div>
        ...
        <TooltipProvider delayDuration={150}>
          <Tooltip>
            <TooltipTrigger asChild>
              <CardTitle className="text-lg leading-tight pr-12 cursor-help select-none">
                {title}
              </CardTitle>
            </TooltipTrigger>
            <TooltipContent
        ...
        <div className="space-y-2">
          {rules.length > 0 ? (
            rules.slice(0, 3).map((rule: string, i: number) => (
              <div
                key={i}
                className="flex items-start gap-3 text-xs text-muted-foreground p-2.5 bg-background/40 border border-foreground/10 rounded-sm"
              >
                <ShieldCheck
                  className={`w-3.5 h-3.5 shrink-0 mt-0.5 ${optimisticActive ? "text-primary" : "text-muted-foreground"}`}
                />
      ...
      <CardFooter className="relative z-[1] pt-4 flex items-center justify-between border-t border-foreground/10 text-[10px] font-bold uppercase tracking-wider text-muted-foreground mt-auto bg-background/30">
        <div className="flex items-center gap-1.5">
          <AlertCircle
            className={`w-3 h-3 ${severity === "CRITICAL" && optimisticActive ? "text-red-400" : ""}`}
            aria-hidden="true"
          />
          {severity}
        </div>
