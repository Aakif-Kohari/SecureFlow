...
vi.mock("@/lib/middleware/rate-limit", () => ({
  withRateLimit: <T>(
    handler: T,
    config: { keyPrefix: string; limit: number; windowSeconds: number },
  ): T => {
...
function makeRequest(
  body: string,
  {
    ...
    extraHeaders = {},
  }: { secret?: string; extraHeaders?: Record<string, string> } = {},
) {
...
describe("/api/webhooks/generic", () => {
  ...
  it("rejects a signature made with the default endpoint secret", async () => {
    ...
    const res = await GENERIC_POST(
      makeRequest(JSON.stringify({ event: "ping" }), {
        secret: "default-secret-key-12345",
      }) as never,
    );
